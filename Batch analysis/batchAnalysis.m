...
for iFile = 1:nFiles
    ...
    if S.general.analysisSteps.doAnalysis(2)
        %% Tracking
        % Tracking was chosen by the user to be included in the batch analysis.
        ...
        % First, build the tracks based on all localizations.
        ...
        if maskSelections(iFile) == true
            ...
            % Build the tracks based on the masked localizations.
            tmpTracks = assembleTracks(pointList(TFin,:), 'maxGapClose', S.tracking.maxGapClose, ...
                'maxLinkDistance', S.tracking.maxLinkDistance, 'exportAllInfo', true);
            ...
            % Remove invalid tracks and overwrite the previous (unmasked) variable |trackArray|.
            isValid = all(trackFeatures >= trackLimits(1,:) & trackFeatures <= trackLimits(2,:), 2);
            trackArray = tmpTracks(isValid);

            % Update the entry that counts the tracks after masking in the 'tracking results' sheet.
        ...
        if S.general.rawDataOutput.("raw data output")(2)
            ...
            T = array2table([trackID,trackList(:,1:3)], 'VariableNames', {'trackID','frame','posX_nm','posY_nm'});
            writetable(T, outputFilename, "Delimiter", "tab");
        end
    end
    ...
    if S.general.analysisSteps.doAnalysis(3)
        ...
        checkCancel(progressDialog);
         ...
         if iFile == 1
            ...
            % If this is the first file and logD histograms are to be exported, create the figure that will be updated
            % when processing each file.
            if S.general.rawDataOutput.("raw data output")(3)
                [hFig, hAx, hPlots] = exportLogDHistogramsInit;
            end
        end
        ...
        try
            ...
            % If the fit with a normal distribution failed (too few data points or none at all), set |D1pop| to 0. The
            % Gaussian mixture model will then definitely fail, producing the fit score "failed". This should give the
            % user enough info to exclude this file in later analysis steps.
            pd = [];
            D1pop = 0;
        end
        ...
        if S.general.rawDataOutput.("raw data output")(3)
            % The user specified that the logD histograms should be stored as .png files.
            [pathname, filename] = fileparts(S.files.inputFiles{iFile});
            pngFilename = fullfile(pathname, strcat(filename, S.general.rawDataOutput.("file suffix")(3), '.png'));

            % Update the (invisible) plot and store it as a .png file.
            hFig = exportLogDHistograms(hFig, hAx, hPlots, pd, pdComp1, pdComp2, fractionComp1, errorCode, filename);
            drawnow;
            exportgraphics(hFig, pngFilename, "BackgroundColor","white");

            % If this was the last file, delete the figure.
            if iFile == nFiles
                delete(hFig)
            end
    ...
    if S.general.analysisSteps.doAnalysis(4)
        ...
        if contains(S.cluster.input, 'track')
            % Use the track centroids as points for the cluster analysis. If a mask was supplied during the tracking
            % procedure, centroids will be based on the masked tracks. NASTIC and segNASTIC will use |trackArray|
            % instead of |pointList|, but |trackArray| might also be masked if specified before.
            pointList = double(convertTracksToCentroids(trackArray));
        else
            % Use the coordinates of the localization results. Masks do not apply here.
...
function [hFig, hAx, hPlots] = exportLogDHistogramsInit

% Initialize a figure and make it invisible.
hFig = figure("Position",[800,600,1000,350],"Color","w","Visible","off");
hTile = tiledlayout(hFig,1,2);
hTile.Title.String = "File: ";
hTile.Title.Interpreter = "none";
hTile.TileSpacing = "tight";
hTile.Padding = "compact";
hTile.YLabel.String = 'occurrence';

% Create tiles for the two plots (single and two-population fit).
hAx(1) = nexttile(hTile);
hAx(2) = nexttile(hTile);

% Format the axes.
for iAx = 1:2
    hAx(iAx).NextPlot = "add";
    hAx(iAx).YLim = [0, 100];
    hAx(iAx).YLimMode = 'auto';
    hAx(iAx).YAxis.TickLabelFormat = '%.0f';
    hAx(iAx).XAxis.Label.String = "logD";
    hAx(iAx).XAxis.LineWidth = 1;
end

% Set plot titles and remove the Y axis ticks and values from the right plot (they are identical to the left plot).
hAx(1).Title.String = "Fit (one population)";
hAx(2).Title.String = "Fit (two populations)";
hAx(2).YAxis.TickValues = [];

% Prepare all plot objects:
% (1) bars for left plot
% (2) bars for right plot
% (3) single-component fit line (left plot)
% (4-5) two-component fit lines (right plot)
% (6-7) right and top box lines (left plot)
% (8-9) right and top box lines (right plot)
% (10) fit results text (left plot)
% (11) fit results text (right plot)
hPlots = gobjects(11,1);

% Format the plot objects.
hPlots(1) = bar(hAx(1),1,1,'BarWidth',1,'FaceColor',[0,0.4118,0.6667]);
hPlots(2) = bar(hAx(2),1,1,'BarWidth',1,'FaceColor',[0,0.4118,0.6667]);
hPlots(3) = line(hAx(1),[0 1],[0 1],'LineWidth',3,'Color',[0.7843,0.3137,0.2353]);
hPlots(4) = line(hAx(2),[0 1],[0 1],'LineWidth',3,'Color',[0.8431,0.7059,0.4118]);
hPlots(5) = line(hAx(2),[0 1],[0 1],'LineWidth',3,'Color',[0.5098,0.7255,0.6275]);
hPlots(10) = text(hAx(1),-1.1,50,"","FontSize",10);
hPlots(11) = text(hAx(2),-1.1,50,"","FontSize",10);

% Set the histogram limits and bin width (hard coded here). Adjust the X axis limits slighty to reduce clutter when
% displaying the axes.
leftEdge = -5; rightEdge = 1; binWidth = 0.2;
hAx(1).XLim = [leftEdge+0.01, rightEdge-0.01];
hAx(2).XLim = [leftEdge+0.01, rightEdge-0.01];

% Create the box lines.
hPlots(6) = line(hAx(1),[rightEdge rightEdge],[0 10],'Color','k','LineWidth',1);
hPlots(7) = line(hAx(1),[leftEdge rightEdge],[10 10],'Color','k','LineWidth',1);
hPlots(8) = line(hAx(2),[rightEdge rightEdge],[0 10],'Color','k','LineWidth',1);
hPlots(9) = line(hAx(2),[leftEdge rightEdge],[10 10],'Color','k','LineWidth',1);

% Pre-calculate some plot data.
xDataCourse = leftEdge+0.5*binWidth:binWidth:rightEdge-0.5*binWidth;
xDataFine = linspace(leftEdge, rightEdge, 5000);
set(hPlots(1), 'XData', xDataCourse, 'YData', ones(1,numel(xDataCourse)));
set(hPlots(2), 'XData', xDataCourse, 'YData', ones(1,numel(xDataCourse)));
for iPlot = 3:5
    set(hPlots(iPlot), 'XData', xDataFine, 'YData', ones(1,numel(xDataFine)));
end

end

function hFig = exportLogDHistograms(hFig, hAx, hPlots, pd, pdComp1, pdComp2, fractionComp1, errorCode, filename)

% Set the filename above the plots.
hFig.Children.Title.String = "File: " + filename;

if isempty(pd)
    % This should not happen, but when everything fails, clear all data from the graphs.
    for iPlot = 1:5
        hPlots(iPlot).YData = NaN(size(hPlots(iPlot).XData));
    end

    % Format them, nevertheless, and return.
    hAx(1).YLim = [0, 100];
    hAx(2).YLim = [0, 100];
    hPlots(6).YData(2) = 100;
    hPlots(7).YData = [100, 100];
    hPlots(8).YData(2) = 100;
    hPlots(9).YData = [100, 100];
    hPlots(10).String = "Fit failed.";
    hPlots(11).String = "Fit failed.";
    hPlots(10).Position(2) = 95;
    hPlots(11).Position(2) = 95;
    return
end

% Calculate and update the histogram bar chart for both plots.
logDlist = pd.InputData.data;
xDataCourse = hPlots(1).XData;
binWidth = mean(diff(xDataCourse));
leftEdge = xDataCourse(1) - 0.5*binWidth;
rightEdge = xDataCourse(end) + 0.5*binWidth;

N = histcounts(logDlist, leftEdge:binWidth:rightEdge, 'Normalization', 'probability');
nCounts = numel(logDlist);
hPlots(1).YData = round(N*nCounts);
hPlots(2).YData = round(N*nCounts);

% Calculate and update the single population fit line and the corresponding text.
popFitX = hPlots(3).XData;
popFitY1 = pdf(pd, popFitX) * nCounts * binWidth;
hPlots(3).YData = popFitY1;
hPlots(10).String = sprintf("logD (peak): %.2f",pd.mu);

if errorCode == 2
    % Two-component fit failed, so do not plot any fit curves into the second plot.
    hPlots(4).YData = NaN(size(hPlots(4).XData));
    hPlots(5).YData = NaN(size(hPlots(5).XData));
    hPlots(11).String = "Fit failed.";
  
else
    % Calculate the two-component fit lines.
    popFitY2comp1 = pdf(pdComp1, popFitX) * fractionComp1 * nCounts * binWidth;
    popFitY2comp2 = pdf(pdComp2, popFitX) * (1-fractionComp1) * nCounts * binWidth;

    % Update the corresponding plot objects.
    hPlots(4).YData = popFitY2comp1;
    hPlots(5).YData = popFitY2comp2;
    rightText = sprintf("logD (peak, slow): %.2f\nlogD (peak, fast): %.2f",pdComp1.mu,pdComp2.mu);
    hPlots(11).String = rightText;
end

% If the two-component fit did not converge, add a line to the text.
if errorCode == 1
    hPlots(11).String = vertcat(hPlots(11).String, {'Fit did not converge.'});
end

% Rescale. Find the common Y data limits and define new upper limits for the Y axis and text Y position.
allY = get(hPlots(1:5), 'YData');
allYLimMax = max(horzcat(allY{:}));
newYLimMax = allYLimMax * 1.05;
newTextPos = allYLimMax * 0.95;

% Set the new limits and positions to the Y axes, box lines and texts.
hAx(1).YLim = [0, newYLimMax];
hAx(2).YLim = [0, newYLimMax];
hPlots(6).YData(2) = newYLimMax;
hPlots(7).YData = [newYLimMax, newYLimMax];
hPlots(8).YData(2) = newYLimMax;
hPlots(9).YData = [newYLimMax, newYLimMax];
hPlots(10).Position(2) = newTextPos;
hPlots(11).Position(2) = newTextPos;
